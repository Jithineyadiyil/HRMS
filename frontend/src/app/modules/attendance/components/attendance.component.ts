...
export class AttendanceComponent implements OnInit, OnDestroy {
  ...
  checkingOut         = false;
  todayError          = '';
  // Timezone note: backend runs in Asia/Riyadh — stored times are already local, no offset needed
  ...
  ngOnInit(): void {
    ...
    this.loadEmployees();
    if (this.isHR()) this.loadDaily(); // pre-load snapshot for Today tab
  }
  ...
  loadToday(): void {
    ...
    this.http.get<any>('/api/v1/attendance/today').subscribe({
      next:  r => {
        ...
        // server_timezone: 'Asia/Riyadh' — stored HH:MM:SS values are already local time
      },
      error: e => {
        this.todayLoading = false;
        this.todayError   = e?.status === 401 ? 'Session expired. Please re-login.' : (e?.error?.message || 'Could not load attendance.');
      },
    });
  }
  ...
  checkIn(): void {
    ...
    this.http.post<any>('/api/v1/attendance/checkin', {}).subscribe({
      next:  r => { this.todayLog = r.log; this.checkingIn = false; this.todayError = ''; },
      error: e => {
        this.checkingIn = false;
        // 422 = already checked in — the response body contains the existing log.
        // Update todayLog so the UI switches to the correct "checked in" state.
        if (e?.status === 422 && e?.error?.log) {
          this.todayLog   = e.error.log;
          const checkedAt = this.fmtTime(e.error.log?.check_in);
          this.todayError = `You have already checked in today at ${checkedAt}`;
        } else {
          this.todayError = e?.error?.message || 'Check-in failed.';
        }
        // Re-fetch today's log to ensure state is fresh (handles auth expiry, stale data etc.)
        this.loadToday();
      },
    });
  }
  ...
  checkOut(): void {
    ...
    this.http.post<any>('/api/v1/attendance/checkout', {}).subscribe({
      next:  r => { this.todayLog = r.log; this.checkingOut = false; this.todayError = ''; },
      error: e => {
        this.checkingOut = false;
        if (e?.status === 422 && e?.error?.log) {
          this.todayLog   = e.error.log;
          const outAt = e.error.log?.check_out ? ` at ${this.fmtTime(e.error.log.check_out)}` : '';
          this.todayError = `You have already checked out today${outAt}`;
        } else {
          this.todayError = e?.error?.message || 'Check-out failed.';
        }
        this.loadToday();
      },
    });
  ...
  /**
   * Format a stored time string ("HH:MM:SS" or "HH:MM") for display.
   *
   * The backend runs in Asia/Riyadh (UTC+3) so all stored times are already
   * local Riyadh time — no offset conversion is needed.
   *
   * @param t  Time string from API (e.g. "09:30:00")
   * @returns  Display string "HH:MM" (e.g. "09:30") or "—" if falsy
   */
  fmtTime(t: string | null | undefined): string {
    if (!t || t === '—') return '—';
    // Accepts HH:MM:SS or HH:MM — slice to first 5 chars gives HH:MM
    return t.slice(0, 5);
  }
